...
2. `skills/post-to-wechat/.env` with `WECHAT_APP_ID=...`
...
Write the credentials to `skills/post-to-wechat/.env`.
```

After collecting the values (prefer a user-input tool, fall back to a numbered prompt per the User Input Tools rule in SKILL.md) and append:
