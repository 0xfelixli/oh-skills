...
export function loadWechatExtendConfig(): WechatExtendConfig {
  ...
  const extendPath = path.join(skillDir, "EXTEND.md");
  try {
...
function resolveCredentialSource(
  ...
  throw new Error(
    ...
    "Set via EXTEND.md account config, environment variables, or the skill's .env file." +
    partialHint
  );
}
...
export function loadCredentials(
  ...
  const skillDir = options.skillDir ?? getSkillDir();
  ...
  if (prefix) {
    ...
    sources.push(
      ...
      buildCredentialSource(`<skill>/.env (${prefixedKeyLabel})`, skillEnv, `${prefix}APP_ID`, `${prefix}APP_SECRET`),
    );
  }

  sources.push(
    buildCredentialSource("process.env", process.env, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
    buildCredentialSource("<skill>/.env", skillEnv, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
  );
